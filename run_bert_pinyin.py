...
import json
from pytorch_transformers.modeling_bert_pinyin import BertForSequenceClassificationPinyin, BertConfig
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                    ...
                    level = logging.INFO)
MODEL_CLASSES = {
    'bert': (BertConfig, BertForSequenceClassificationPinyin, BertTokenizer),
}
...
def main():
    ...
    config = BertConfig.from_pretrained(args.model_name_or_path, num_labels=2, output_hidden_states=True, pinyin_vocabsize=len(pinyin_dict.keys()))
    
    # Prepare model
    model = BertForSequenceClassificationPinyin.from_pretrained(args.model_name_or_path,args,config=config)
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label, pyid_list)
        print('结束执行拼音')
        if args.local_rank == -1:
        ...
        for step in bar:
            ...
            input_ids, input_mask, segment_ids, label_ids, pyid_list = batch
            loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids, pinyin_ids=pyid_list)
            if args.n_gpu > 1:
            ...
            if args.do_eval and (step + 1) %(args.eval_steps*args.gradient_accumulation_steps)==0:
                for file in ['dev.csv']:
                    ...
                    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label,  pyid_list)
                    ...
                    for input_ids, input_mask, segment_ids, label_ids, pyid_list in eval_dataloader:
                        ...
                        with torch.no_grad():
                            tmp_eval_loss= model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids, pinyin_ids=pyid_list)
                            logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, pinyin_ids=pyid_list)
    ...
    if args.do_test:
        ...
        gc.collect()
        args.do_train=False
        model = BertForSequenceClassificationPinyin.from_pretrained(os.path.join(args.output_dir, "pytorch_model.bin"),args,config=config)
        if args.fp16:
            model.half()
        model.to(device)
        if args.local_rank != -1:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            for input_ids, input_mask, segment_ids, label_ids, pyid_list in eval_dataloader:
                ...
                with torch.no_grad():
                    logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask,  pinyin_ids=pyid_list).detach().cpu().numpy()
                label_ids = label_ids.to('cpu').numpy()
