...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, pinyin_ids=None):
        ...
        embeddings = words_embeddings + position_embeddings + token_type_embeddings + pinyin_embeddings
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, head_mask=None, pinyin_ids=None):
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids, token_type_ids=token_type_ids, pinyin_ids=pinyin_ids)
...
@add_start_docstrings("""Bert Model transformer with a sequence classification/regression head on top (a linear layer on top of
    the pooled output) e.g. for GLUE tasks. """,
                      BERT_START_DOCSTRING, BERT_INPUTS_DOCSTRING)
class BertForSequenceClassificationPinyin(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels - 1]``.
    ...
    def __init__(self, config):
        super(BertForSequenceClassificationPinyin, self).__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
                word2vec1=None, word2vec2=None, pinyin_ids=None, position_ids=None, head_mask=None):
        ...
        outputs = self.bert(input_ids=flat_input_ids, position_ids=flat_position_ids,
                            token_type_ids=flat_token_type_ids,
                            attention_mask=flat_attention_mask, head_mask=head_mask, pinyin_ids=flat_pinyin_ids)
        pooled_output = outputs[1]
